...
class dataframe:
	...
	def setSqlEngine(self, creds):
		constr = 'mysql+mysqldb://{user}:{pwd}@{host}:{port}/{db}'.format(user=creds['User'], pwd=creds['Password'], 
		host=creds['Host'], port=creds['Port'], db=creds['Database'])
